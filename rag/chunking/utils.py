...
def kill_processes_on_ports(
    ports: Iterable[int],
    *,
    only_listening: bool = True,  # kill only listeners (recommended)
    include_udp: bool = True,     # also consider UDP sockets
    force: bool = True,           # send SIGKILL/kill() if not gone after timeout
    timeout: float = 5.0          # seconds to wait after terminate() before kill()
) -> Dict[str, Any]:
    """
